...
/**
 * Implementation of simulated annealing to classify breast cancer dataset.
 * 
...
public class SA4BC {
	...
	private static int inputLayer = 30, hiddenLayer = 22, outputLayer = 1, trainingIterations = 1200;
	...
	public static void main(String[] args) {
		...
		//for (;temp > 10; temp /=10) {
			...
			for (int i = 0; i < repeat; i++) {
				List<Double> result = oneRun(temp, rate);
			...
			System.out.println(df.format(trainingAcc));
			System.out.println(df.format(testingAcc));
			System.out.println(df.format(trainingTime));
			System.out.println(df.format(testingTime));
		}
	//}

	private static List<Double> oneRun(double temp, double rate) {
		...
		oa = new SimulatedAnnealing(temp, rate, nnop);
